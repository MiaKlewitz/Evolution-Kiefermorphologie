...
# HULLS #
xfind_hull.all <- function(all.func) all.func[chull(all.func$PC5, all.func$PC6), ]
xfind_hull <- function(NA.func.all) NA.func.all[chull(NA.func.all$PC5, NA.func.all$PC6), ]
...
# zhulls #
zfind_hull.rod <- function(NA.func.rodent) NA.func.rodent[chull(NA.func.rodent$PC5, NA.func.rodent$PC6), ]
...
# Plotting
# -----------
plot1 <- ggplot(data = NA.func.all, aes(PC5, PC6, fill = Ordnung, pch = Ordnung, size = Status, label=Spezies))+ #, label=Spezies))+
...
# ---------------------------
plot2 <- ggplot(data = NA.func.rodent, aes(PC5, PC6, col = Ordnung, fill = Ordnung, pch = Ordnung, size = Status, label=Spezies))+
...
# ---------------------------
combined_plot1 <- plot1 + plot_layout(guides = "collect") & theme(legend.position = "bottom") 
combined_plot2 <- plot2
